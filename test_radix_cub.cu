...
// 测试标准版的radixSelectTopKCub方法
float testRadixSelectTopKCub(half* h_input, half* h_output, uint* h_indices, 
    ...
    // 执行TopK
    cudaError_t error = radixSelectTopKCub<half>(
        d_input, num_items, k, d_output, d_indices, g_allocator);
    ...
    if (run % 10 == 0) {
        printf("radixSelectTopKCub GPU计算的前10个值: ");
    ...
    if (validate) {
        ...
        if (success) {
            printf("验证成功! radixSelectTopKCub 结果与索引均正确\n");
        } else {
            printf("验证失败! radixSelectTopKCub 结果或索引不正确\n");
        }
    }
...
// 测试预分配版的radixSelectTopKPreallocatedCub方法
float testRadixSelectTopKPreallocatedCub(half* h_input, half* h_output, uint* h_indices, 
                                    half* d_input, half* d_output, uint* d_indices, 
                                    ValueIndexPair* h_reference_pairs,
                                    cub::CachingDeviceAllocator& g_allocator,
                                    int num_items, int k, int run, bool validate) {
    ...
    CubDebugExit(g_allocator.DeviceAllocate((void **)&d_keys_sorted, sizeof(half) * num_items));
    ...
    // 执行TopK
    cudaError_t error = radixSelectTopKPreallocatedCub<half>(
        d_input, num_items, k, d_output, d_indices, 
        d_keys_sorted, d_indices_in, d_indices_sorted, g_allocator);
    ...
    if (run % 10 == 0) {
        printf("radixSelectTopKPreallocatedCub GPU计算的前10个值: ");
        for (int i = 0; i < std::min(10, k); i++) {
            printf("%f(索引%d) ", __half2float(h_output[i]), h_indices[i]);
        }
        printf("\n");
    ...
    if (validate) {
        bool success = validateResults(h_output, h_indices, h_reference_pairs, h_input, k);
        if (success) {
            printf("验证成功! radixSelectTopKPreallocatedCub 结果与索引均正确\n");
        } else {
            printf("验证失败! radixSelectTopKPreallocatedCub 结果或索引不正确\n");
        }
    }
...
{
    // 创建CUB分配器
    cub::CachingDeviceAllocator g_allocator(true);  // 启用调试模式
    ...
    printf("开始测试 radixSelectTopKCub 和 radixSelectTopKPreallocatedCub...\n");
    ...
    for (int run = 0; run < num_runs; run++)
    {
        ...
        // 测试标准版本
        float time_standard = testRadixSelectTopKCub(h_input, h_output, h_indices, 
                                               d_input, d_output, d_indices, 
                                               h_reference_pairs, g_allocator,
                                               num_items, k, run, 
                                               validate && (run % 10 == 0));
        total_time_standard += time_standard;
        
        // 测试预分配版本
        float time_preallocated = testRadixSelectTopKPreallocatedCub(h_input, h_output, h_indices, 
                                                              d_input, d_output, d_indices, 
                                                              h_reference_pairs, g_allocator,
                                                              num_items, k, run, 
                                                              validate && (run % 10 == 0));
        ...
        if (run % 10 == 0) {
            printf("运行 %d: radixSelectTopKCub 用时: %.3f ms, radixSelectTopKPreallocatedCub 用时: %.3f ms\n", 
    ...
    printf("\n性能总结 (%d 次运行):\n", num_runs);
    printf("radixSelectTopKCub 平均用时: %.3f ms\n", avg_time_standard);
    printf("radixSelectTopKPreallocatedCub 平均用时: %.3f ms\n", avg_time_preallocated);
